...
logits = model(inp)
filePath = "../my_roseNNa_examples/Ld/"
with open(filePath+"Ld.txt", "w") as f:
...
torch.onnx.export(model,               # model being run
                  ...
                  filePath+"Ld.onnx",   # where to save the model (can be a file or file-like object)
...
torch.onnx.export(model,               # model being run
                  inp,                         # model input (or a tuple for multiple inputs)
                  filePath+"Ld_weights.onnx",   # where to save the model (can be a file or file-like object)
                  export_params=True,        # store the trained parameter weights inside the model file
                  opset_version=10,          # the ONNX version to export the model to
